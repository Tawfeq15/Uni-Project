...
class ExamScheduleImportService
{
    ...
    public function previewFile($file, array $importData)
    {
        ...
        if (!empty($importData['column_mapping'])) {
            ...
            if (!isset($headerMap['course_code']))
                $missingRequired[] = 'كود المادة';
            if (!isset($headerMap['date']))
                $missingRequired[] = 'التاريخ';
            if (!isset($headerMap['rooms']))
                $missingRequired[] = 'القاعات';
            if (!isset($headerMap['start_time']) && !isset($headerMap['time']))
                $missingRequired[] = 'الوقت';
        ...
        $lastStartTime = null;
        $lastEndTime = null;
        $lastCourseCode = null;   // FIX: carry-forward course_code for multi-section rows
        $lastCourseName = null;   // FIX: carry-forward course_name for multi-section rows
        ...
        foreach ($rows as $index => $row) {
            ...
            if (empty($rowData['day']) && $lastDay)
                $rowData['day'] = $lastDay;
            if (empty($rowData['date']) && $lastDate)
                $rowData['date'] = $lastDate;
            if (empty($rowData['time']) && empty($rowData['start_time']) && empty($rowData['end_time'])) {
                if ($lastStartTime)
                    $rowData['start_time'] = $lastStartTime;
                if ($lastEndTime)
                    $rowData['end_time'] = $lastEndTime;
            }
            // FIX: carry-forward course_code and course_name for multi-section rows
            if (empty($rowData['course_code']) && $lastCourseCode)
                $rowData['course_code'] = $lastCourseCode;
            if (empty($rowData['course_name']) && $lastCourseName)
                $rowData['course_name'] = $lastCourseName;

            // Update carry forward state ALWAYS, even if we are going to skip the row
            // This is crucial because a skipped row might be the first row of a merged date cell
            if (!empty($rowData['day']))
                $lastDay = $rowData['day'];
            if (!empty($rowData['date']))
                $lastDate = $rowData['date'];
            // FIX: update course carry-forward
            if (!empty($rowData['course_code']))
                $lastCourseCode = $rowData['course_code'];
            if (!empty($rowData['course_name']))
                $lastCourseName = $rowData['course_name'];
            ...
            if (!empty($rowData['time'])) {
                ...
                if ($timeObj['start_time'])
                    $rowData['start_time'] = $timeObj['start_time'];
                if ($timeObj['end_time'])
                    $rowData['end_time'] = $timeObj['end_time'];
            }

            if (!empty($rowData['start_time']))
                $lastStartTime = $rowData['start_time'];
            if (!empty($rowData['end_time']))
                $lastEndTime = $rowData['end_time'];



            $validation = $this->validateRow($rowData, $importData);

            // Skip holiday rows completely so they don't appear as errors
            if ($validation['status'] === 'skipped') {
                $totalCount--; // Subtract from total since we incremented it earlier
                continue;
            }
        ...
        foreach ($parsedRows as &$pRow) {
            ...
            elseif ($validation['status'] === 'warning')
                $warningCount++;
            elseif ($validation['status'] === 'conflict')
                $conflictCount++;
            else
                $invalidCount++;
        }
    ...
    private function mapHeaders(array $headers): array
    {
        ...
        foreach ($headers as $index => $header) {
            ...
            if (in_array($header, ['course_code', 'كود المادة', 'رمز المقرر', 'رقم المادة', 'رمز المادة', 'رقم المقرر', 'subject_code']))
                $map['course_code'] = $index;
            elseif (in_array($header, ['course name', 'course_name', 'اسم المادة', 'اسم المقرر', 'subject_name']))
                $map['course_name'] = $index;
            elseif (in_array($header, ['section', 'sections', 'الشعبة', 'الشعب', 'ش']))
                $map['sections'] = $index;
            elseif (in_array($header, ['instructor', 'instructors', 'المحاضر', 'المدرس', 'مدرس المادة', 'lecturer', 'اسم المحاضر']))
                $map['instructors'] = $index;
            elseif (in_array($header, ['date', 'التاريخ', 'تاريخ']))
                $map['date'] = $index;
            elseif (in_array($header, ['day', 'اليوم', 'يوم']))
                $map['day'] = $index;
            elseif (in_array($header, ['time', 'الوقت', 'وقت']))
                $map['time'] = $index;
            elseif (in_array($header, ['start_time', 'start', 'from', 'من', 'الوقت من', 'وقت البداية']))
                $map['start_time'] = $index;
            elseif (in_array($header, ['end_time', 'end', 'to', 'إلى', 'الوقت إلى', 'وقت النهاية']))
                $map['end_time'] = $index;
            elseif (in_array($header, ['duration', 'المدة']))
                $map['duration'] = $index;
            elseif (in_array($header, ['room', 'rooms', 'القاعة', 'القاعات', 'المختبر', 'المختبرات', 'lab']))
                $map['rooms'] = $index;
            elseif (in_array($header, ['capacity', 'السعة']))
                $map['capacity'] = $index;
            elseif (in_array($header, ['students', 'student_count', 'الطلبة', 'عدد الطلاب', 'عدد الطلبة المتقدمين', 'عدد الطلبة']))
                $map['student_count'] = $index;
            elseif (in_array($header, ['المتقدمين', 'المتقدمين للامتحان', 'عدد المتقدمين', 'actual_students']))
                $map['actual_students'] = $index;
            elseif (in_array($header, ['notes', 'ملاحظات']))
                $map['notes'] = $index;
        }
        return $map;
    }
    ...
    private function extractRowData(array $row, array $map): array
    {
        $extract = function ($key) use ($row, $map) {
            ...
            $val = $row[$map[$key]];
            if (is_string($val)) {
                // FIX: strip \xa0 (non-breaking space) which trim() misses
                $val = preg_replace('/\xc2\xa0|\x{00A0}/u', ' ', $val);
                $val = trim($val);
            }
            return $val !== '' ? $val : null;
        };
        ...
        $rooms = [];
        if ($roomsStr) {
            // Replace slashes and parentheses with commas so reserve rooms like /(2106) are separated
            $roomsStr = str_replace(['/', '(', ')', '\\'], ',', $roomsStr);
            ...
            foreach ($rooms as $r) {
                ...
                // Do not split room codes that contain letters, e.g. IT-LAB
                if (preg_match('/[\p{L}]/u', $r)) {
                    $finalRooms[] = $r;
                } else {
            ...
            $rooms = array_values(array_filter($finalRooms, fn($v) => $v !== ''));
        }
        ...
        return [
            ...
            // FIX: prefer المتقدمين (actual exam takers) over عدد الطلبة (enrolled) if available
            'student_count' => (int) ($extract('actual_students') ?? $extract('student_count') ?? 0),
            'enrolled_count' => (int) ($extract('student_count') ?? 0),
    ...
    private function validateRow(array $data, array $importData): array
    {
        ...
        $isHoliday = false;
        // FIX: also check time field — holidays sometimes written there (e.g. "احد الشعانين")
        foreach (array_merge([$data['course_code'], $data['course_name'], $data['notes'], $data['time'] ?? null], $data['rooms']) as $val) {
            if (empty($val))
                continue;
            foreach ($holidayKeywords as $kw) {
                if (mb_stripos((string) $val, $kw) !== false) {
                    $isHoliday = true;
                    break 2;
                }
            }
        ...
        if (empty($data['course_code']))
            $errors[] = "كود المادة مفقود";
        if (empty($data['section_numbers']) && !$isHoliday)
            $warnings[] = "الشعب مفقودة";
        if (empty($data['rooms']) && !$isHoliday)
            $errors[] = "القاعات مفقودة";
        ...
        // FIX: if date was auto-corrected (M/D ↔ D/M swap), update normalized_date
        if (!empty($dayResolve['corrected_date'])) {
            $normDate = $dayResolve['corrected_date'];
        }
        if ($dayResolve['warning'])
            $warnings[] = $dayResolve['warning'];
        if (!$day && !$isHoliday)
            $errors[] = "اليوم غير صالح أو مفقود";

        $duration = 60;
        if ($normStart && $normEnd) {
            $s = Carbon::parse($normStart);
            $e = Carbon::parse($normEnd);
            // FIX: use abs() to prevent negative durations from RTL time inversion
            $duration = abs($e->diffInMinutes($s));
            if ($duration === 0)
                $duration = 60; // fallback if start == end
        }
    ...
    private function checkInternalConflict(array $currentRow, array $allRows): ?string
    {
        ...
        if (!$cVal['normalized_date'] || !$cVal['normalized_start'] || !$cVal['normalized_end'])
            return null;
        ...
        foreach ($allRows as $pRow) {
            if ($pRow['index'] === $currentRow['index'])
                continue; // skip self

            $pVal = $pRow['validation'];
            $pRowD = $pRow['rowData'];

            if ($pVal['status'] === 'invalid')
                continue;

            if ($cVal['normalized_date'] === $pVal['normalized_date']) {
                if (!$pVal['normalized_start'] || !$pVal['normalized_end'])
                    continue;
